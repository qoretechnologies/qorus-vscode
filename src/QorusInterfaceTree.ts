...
class QorusInterfaceTree implements TreeDataProvider<QorusInterfaceTreeNode> {
  ...
  private data: any;
  private is_folder_view: boolean = true;
  ...
  private onTreeDataChanged: EventEmitter<QorusInterfaceTreeNode | undefined> = new EventEmitter<
    QorusInterfaceTreeNode | undefined
  >();
    ...
    if (this.is_folder_view) {
      ...
      if (this.code_info) {
        ...
        for (const dir of fileTree) {
          children.push(
            new QorusTreeDirectoryNode(this.code_info, dir.rel_path, true, dir, this.extensionPath)
          );
        }
      }
    } else {
...
class QorusTreeDirectoryNode extends QorusInterfaceTreeNode {
  ...
  async getChildren(_node?: QorusInterfaceTreeNode): Promise<QorusInterfaceTreeNode[]> {
    ...
    const dirs = this.fileTree.dirs.sort((a, b) =>
      basename(a.rel_path).localeCompare(basename(b.rel_path))
    );
    for (const dir of dirs) {
      children.push(
        new QorusTreeDirectoryNode(this.code_info, dir.rel_path, false, dir, this.extensionPath)
      );
    }
