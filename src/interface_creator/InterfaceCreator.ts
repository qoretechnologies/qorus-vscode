...
export abstract class InterfaceCreator {
  ...
  /**
   * It checks if the interface already exists.
   */
  protected checkExistingInterface = (params: any): any => {
    ...
    /**
         * This code is checking to see if the edit_type is create or edit. If it is not, it returns
        true.
         */
    ...
    /**
     * If the interface name is different from the original interface name,
     * and the new interface name already exists, then the interface name is not valid.
     */
    const {
      name: orig_name,
  ...
  protected renameClassAndBaseClass(lines: string[], orig_data: any, header_data): string[] {
    ...
    const eraseBaseClassName = (without_space: boolean = false) => {
      ...
      const length =
        main_base_class_name_range.end.character - main_base_class_name_range.start.character;
        ...
        if (item[tag] !== undefined && (!item.parent_data || item.parent_data[tag] !== item[tag])) {
          ...
          if (['list', 'hash'].includes(non_star_type)) {
            ...
            if (non_star_type === 'list') {
              ...
              result +=
                lines.map((str) => `${indent.repeat(indent_level + 2)}${str}`).join('\n') + '\n';
            }
          } else {
        ...
        if (
          ...
          if (Array.isArray(item[tag])) {
            ...
            for (let entry of item[tag]) {
              result += `${indent.repeat(indent_level + 1)}${list_indent}${JSON.stringify(
                entry
              )}\n`;
            }
          } else {
      ...
      if (Array.isArray(value)) {
          ...
          case 'class-connectors':
            for (const connector of value) {
              ...
              for (const key in connector) {
                if (
                  ['provider', 'input-provider', 'output-provider'].includes(key) &&
                  connector[key]
                ) {
        ...
        switch (tag) {
          ...
          case 'output-type':
            ...
            result +=
              `${
                ['mapper_options', 'fsm_options', 'connection_options'].includes(tag)
                  ? 'options'
                  : tag
              }:\n` + InterfaceCreator.indentYamlDump(fixed_value, 1, true);
            break;
