...
export const triggers = (
    ...
    { iface_kind, 'base-class-name': base_class = undefined }
) => {
    switch (iface_kind) {
        case 'service':
        ...
        case 'job':
            return ['run'];
        case 'step':
            return Object.keys(stepTriggerSignatures(code_info, base_class)); // only keys: does not depend on lang
        default:
            return [];
    }
};
        ...
        switch (code_info.stepType(base_class)) {
            ...
            case 'QorusAsyncArrayStep':
                return {
                    primary: { signature: 'primary(auto array_arg)', arg_names: ['array_arg'] },
                    validation: {
                        signature: 'string validation(*string async_key, auto array_arg)',
                        arg_names: ['async_key', 'array_arg'],
                    },
        ...
        switch (code_info.stepType(base_class)) {
            ...
            case 'QorusAsyncArrayStep':
                return {
                    primary: { signature: 'primary(self, array_arg)', arg_names: ['array_arg'] },
                    validation: {
                        signature: 'validation(self, async_key, array_arg)',
                        arg_names: ['async_key', 'array_arg'],
                    },
        ...
        switch (code_info.stepType(base_class)) {
            ...
            case 'QorusAsyncArrayStep':
                return fixSignature({
                    primary: { signature: 'void primary(Object array_arg)', arg_names: ['array_arg'] },
                    validation: {
                        signature: 'String validation(String async_key, Object array_arg)',
                        arg_names: ['async_key', 'array_arg'],
                    },
                    end: {
                        signature: 'void end(Object queue_data, Object array_arg)',
                        arg_names: ['queue_data', 'array_arg'],
                    },
                    array: { signature: 'Object[] array()' },
                });
            default:
