...
import { QorusTreeInstanceNode, instance_tree } from './QorusInstanceTree';
import { hosted_instance_token, hosted_instance_url, projects } from './QorusProject';
...
export class QorusLogin extends QorusAuth {
  ...
  logout(tree_item: vscode.TreeItem) {
    const url: string =
      typeof tree_item === 'string' ? tree_item : (<QorusTreeInstanceNode>tree_item).getUrl();
  ...
  requestError(error_data: any, default_error: string) {
    ...
    if (error_data.statusCode == 401) {
      ...
      this.doLogout();
    } else if (error_data.message?.indexOf('authenticate') > -1 && error_data.statusCode == 400) {
