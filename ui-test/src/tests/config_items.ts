...
export const editInterface = async (workbench: Workbench, project_folder: string) => {
    await sleep(1000);
    await openInterface(workbench, path.join(project_folder, target_dir, 'ClassWithConfigItems-1.3.qclass.yaml'));
    await sleep(4000);
    // more to do
};
