...
export const getQorusTreeSection = async (sectionName: string): Promise<CustomTreeSection> => {
  ...
  while (!qorusTreeSection) {
    qorusTreeSection = (await new SideBarView()
      .getContent()
      .getSection(sectionName)) as CustomTreeSection;
    await sleep(1000);
  }
...
export const getQorusTreeItem = async (
  sectionName: string,
  itemName: string
): Promise<TreeItem> => {
...
export const loginFromTreeView = async (workbench: Workbench, instanceName: string) => {
  ...
  if (instance) {
    const loginAction: TreeItem = (await instance.findChildItem(
      'Set as active Qorus instance'
    )) as TreeItem;
