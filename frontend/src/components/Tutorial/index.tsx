...
import { ReqoreButton, ReqoreControlGroup, ReqoreIcon } from '@qoretechnologies/reqore';
...
const Tutorial = ({ data, onClose }) => {
  ...
  const contentAndStyles = useMemo(() => {
    ...
    const { text, title, elementData }: { text: string; title: string; elementData: DOMRect } =
      data[activeStep];
      ...
      <StyledTutorialWrapper>
        ...
        >
          ...
          {activeStep === data.length - 1 ? (
            ...
            <ReqoreButton
              tooltip={autoPlay ? t('PauseTutorial') : t('PlayTutorial')}
              icon={autoPlay ? 'PauseLine' : 'PlayLine'}
              onClick={() => setAutoPlay(!autoPlay)}
              intent={autoPlay ? 'success' : 'info'}
            />
          )}
          <ReqoreButton icon="CloseLine" onClick={onClose} tooltip={t('CloseTutorial')} />
        </ReqoreControlGroup>
        <StyledTutorialBox
          ref={measuredRef}
        ...
        >
          <StyledBoxHeader id="tutorial-box-title">
            <span>
              <ReqoreIcon icon="InformationLine" intent="info" /> {t(title)}
            </span>
            <span>
