...
class EditPopover extends Component {
    ...
    setOpen = is_open => {
        this.props.setOpen(this.id, is_open);
    };
    ...
    render() {
        ...
        const UrlLabel = ['edit-main-url', 'edit-qorus'].includes(this.action) ? t('MainUrl') : t('Url');
        ...
        return (
            <Popover
                ...
                onInteraction={nextOpenState => {
                    this.setOpen(nextOpenState);
                }}
                isOpen={this.props.is_open}
            >
                <Button icon={kind == 'edit' ? 'edit' : 'plus'} title={kind == 'edit' ? t('Edit') : t('AddNew')} />
...
export const EditPopoverContainer = compose(
    connect(
        mapStateToProps,
        mapDispatchToProps
    ),
